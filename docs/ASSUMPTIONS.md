...
### Labour-market treatment of migrants (known limitation)

- **Current behaviour**: Migrants added to the population are treated identically
  to the rest of the population in the labour market. The model applies the same
  `employmentRate` to the whole working-age population and the same `avgWage` to
  all employed persons. In effect, v0.2.0 assumes that migrants are employed at
  exactly the same rate, and earn exactly the same average wage, as the
  native-born population from their first year.
- **Why this matters**: This is a strong assumption. In reality, employment rates
  and earnings of foreign-born residents differ from the native average,
  particularly in the years following arrival. Assuming immediate full
  integration overstates the contribution base generated by migration and
  therefore the fiscal relief migration provides to the pension system.
- **Status**: This is a **documented simplification**, not a data-driven estimate.
  No separate migrant employment or earnings data are currently used, and no
  number has been invented to represent them.
- **Relationship to the specification**: `docs/SPEC.md` §7.6 states that the effect
  of migration on employment "should be mediated through the employment model
  rather than treating every migrant as immediately employed." v0.2.0 does not yet
  meet this requirement; the discrepancy is recorded here.
- **Possible future treatment**: Track a separate migrant cohort, apply a distinct
  employment rate and (optionally) a relative wage to it, and document the source
  for any such figure. This would be a model-version change.
